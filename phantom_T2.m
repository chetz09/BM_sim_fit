...
disp('Select T2 scan');
DIR = uigetdir(pwd, 'Select T2 map containing dicom files which has .dcm');
...
for i = 1:numFiles
    ...
    if isfield(info, 'EchoTime')
        ...
        warning('No TE found in %s', dicomFiles(i).name);
        TEs(i) = NaN;
    end
end
%% Automatically detect the phantom outline instead of drawing manually
Sa = dicomVolume(:,:,8);  % First slice
...
figure; imagesc(dicomVolume(:,:,8)); colormap("gray"); axis image off; 
...
% Preallocate
T2map = zeros(dim(1), dim(2), numTubes);
M0_2 = zeros(dim(1), dim(2), numTubes);
T2_means = zeros(numTubes,1);


disp("Starting T2 mapping ...");
    ...
    for i = 1:dim(1)
        for j = 1:dim(2)
            if currentMask(i,j)
                ...
                [M0_2(i,j,t), T2map(i,j,t)] = t2fitting_new(S, TEs);
            end
        end
    end

    meanSignal = zeros(length(TEs),1);
    for k = 1:length(TEs)
        tmpImg = dicomVolume_masked(:,:,k);
        meanSignal(k) = mean(tmpImg(currentMask), 'omitnan');  % mean over mask
    end

    % --- Fit mean signal for ROI using t2fitting_VTR ---
    [M0_fit, T2_fit, c_fit, S_fit] = t2fitting_new(meanSignal, TEs);

    subplot(4,6,t);
    scatter(TEs, meanSignal, 40, 'filled', 'MarkerFaceColor',[0.1 0.7 1]); hold on;
    plot(TEs, S_fit, 'r-', 'LineWidth', 1.5);

    xlabel('TE (ms)'); ylabel('Signal');
    title(sprintf('Tube %d: T1 = %.0f ms', t, T2_fit));
    legend('Data','Fit','Location','southeast');
    
    % Mean T2 only for current tube
    maskedT2 = T2map(:,:,t);
    T2_means(t) = mean(maskedT2(currentMask & maskedT2>0 & maskedT2<1000)); 
    % thresholds remove nonsense fits if needed
end

disp("Done!");

compositeT2 = zeros(size(tubeMasks(:,:,1)));

for t = 1:numTubes
    tubeMask = tubeMasks(:,:,t) > 0;
    T2tube   = T2map(:,:,t);
    compositeT2(tubeMask) = T2tube(tubeMask); % keep voxelwise T2 values
end
...
figure;
ax1 = axes;
imagesc(ax1, dicomVolume(:,:,1));  % anatomical background
axis image off;
colormap(ax1, 'gray');

ax2 = axes;
imagesc(ax2, compositeT2, 'AlphaData', compositeT2 > 0); % overlay T1 map
axis image off;
load('T2cm.mat');
...
colorbar;
set(gca, 'FontSize', 18);
saveas(gcf, 'T2map_37.tiff');

T2_means_table = array2table(T2_means, ...
    'VariableNames', {'Mean_T2_ms'});
writetable(T2_means_table, 'T2_means.xlsx');
